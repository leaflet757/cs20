...
Entities.add('enemy_direct_suicider',Entities.create({
	...
	update: function(state,delta){
		if(state.inActiveScope){
			...
			state.accelerateToward(p.cx-state.width/2,p.cy-state.height/2,100+ 300/dist);
		}
	},
	destroy: function(state){
		state.deathSound.play(0)
		Entities.shrink_burst.burst(8,state.x+state.width/2,state.y+state.height/2,18,18,12,200,1,0,0,0.1,state.vel[0],state.vel[1]);
	}
}));